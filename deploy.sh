...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "💡 You can use our setup script to configure these environment variables:"
    echo "   node scripts/setup_vercel_env.js"
    echo ""
    ...
    echo "   2. The cron jobs are already configured in vercel.json:"
    echo "      - Daily broadcast at 12:00 UTC: /api/admin/scheduled-broadcast"
    echo "      - TON balance updates every 6 hours: /api/cron/update-ton-balances"
    echo "   3. Make sure the CRON_SECRET_KEY environment variable is set to a secure random string"
    echo "   4. You can monitor cron job executions in the cron_logs table"
    echo ""
